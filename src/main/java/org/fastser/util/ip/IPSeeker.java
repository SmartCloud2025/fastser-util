...
public class IPSeeker {
    ...
    private static File IP_FILE = null;
 
    // 一些固定常量，比如记录长度等等
    private static final int IP_RECORD_LENGTH = 15;
     ...
     * 私有构造函数
     */
    private IPSeeker(String path)  {
    	IP_FILE = new File(path);
        ...
        if(ipFile != null) {
            try {
                ipBegin = 0;//readLong4(0);
                ipEnd = ipFile.length();//readLong4(4);
     ...
     * @return 单一实例
     */
    public static IPSeeker getInstance(String path) {
    	if(null == instance){
	...
	private byte[] readIP(long offset) {
		byte[] ip = new byte[4];
	    try {
	        if(offset > 0){
	        	for(;offset > 0;offset--){
    ...
    private IPLocation getIPLocation(long offset) {
        try {
            ...
            if(b == AREA_FOLLOWED) {
                ...
                b = ipFile.readByte();
                if(b == NO_AREA) {
                    loc.country = readString(readLong3())[0];
                    ipFile.seek(countryOffset + 4);
                } else
                    loc.country = readString(countryOffset)[0];
                // 读取地区标志
                loc.area = readArea(ipFile.getFilePointer());
            } else if(b == NO_AREA) {
            	String[] vals = readString(readLong3());
                loc.country = vals[0];
                loc.area = vals[1];
            } else {
            	String[] vals = readString(offset);
                loc.country = vals[2];
                loc.area = vals[3];
            }
            return loc;
    ...
    private String readArea(long offset) throws IOException {
        ...
        if(b == 0x01 || b == 0x02) {
            ...
            if(areaOffset == 0)
                ...
                return readString(areaOffset)[1];
        } else
            return readString(offset)[1];
    }
